...
def run_gui_folder_dialog(title: str = "Chon thu muc") -> tuple[str, Optional[str]]:
    ...
    # 4. Fallback to Tkinter subprocess only when not running in frozen PyInstaller mode
...
def run_gui_file_dialog(
    title: str = "Chon file",
    win_ctypes_filter: str = "All Files\0*.*\0\0",
    linux_filter: str = "*.*",
    mac_types: str = "",
...
def select_proxy_file():
    ...
    status, path_or_err = run_gui_file_dialog(
        title=dialog_title,
        win_ctypes_filter="Text files (*.txt)\0*.txt\0All files (*.*)\0*.*\0\0",
        linux_filter="*.txt",
        mac_types='"txt"',
...
def select_video_file():
    ...
    status, path_or_err = run_gui_file_dialog(
        title=dialog_title,
        win_ctypes_filter="Video files (*.mp4;*.mkv;*.mov;*.avi;*.webm;*.m4v)\0*.mp4;*.mkv;*.mov;*.avi;*.webm;*.m4v\0All files (*.*)\0*.*\0\0",
...
def select_srt_file():
    ...
    status, path_or_err = run_gui_file_dialog(
        title=dialog_title,
        win_ctypes_filter="SRT files (*.srt)\0*.srt\0All files (*.*)\0*.*\0\0",
        linux_filter="*.srt",
        mac_types='"srt"',
