...
class TestSubtitleModule(unittest.TestCase):
    ...
    def test_whisper_monotonic_milestones(self):
        ...
        progress_calls = []
        def my_progress_cb(pct, phase=""):
            progress_calls.append((pct, phase))

        # Test Case A: Runner supports progress_callback. Milestone must only be forwarded (not duplicated).
        def fake_runner_with_cb(video_path, config, progress_callback=None):
        ...
        # Test Case B: Runner does NOT support progress_callback. Provider generates milestones for backward-compatibility.
        progress_calls.clear()
        def fake_legacy_runner(video_path, config):
            return SubtitleProviderResult(segments=[], metadata={})
    ...
    def test_whisper_cooperative_cancel(self):
        from modules.subtitle.providers import SubtitleProviderResult, WhisperSubtitleProvider
        from unittest.mock import MagicMock
        import threading

        # Triggered check 1: before load
        cancel_event = threading.Event()
        cancel_event.set()
        ...
        self.assertFalse(res.success)
        self.assertTrue(res.canceled)
        self.assertIn("Canceled", res.error)
